            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($results['ranking'] as $altId => $rank)
                        <tr class="@if($loop->odd) bg-gray-50 dark:bg-gray-700 @endif">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                {{ $rank }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                                {{ $results['alternatifs']->find($altId)->nama }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold 
                                @if($results['netFlow'][$altId] > 0) text-green-600 dark:text-green-400 
                                @elseif($results['netFlow'][$altId] < 0) text-red-600 dark:text-red-400 
                                @else text-gray-600 dark:text-gray-400 @endif">
                                {{ number_format($results['netFlow'][$altId], 4) }}
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
